...
After you have followed either of the options above, enter the 6-digit verification code generated by the Google Authenticator app when prompted. _Note that the code and your password will not be visible on the screen when typed._ If the code is correct, the system will then prompt you for your password. 
	*If you make a mistake entering the code, you will be prompted to enter the verification code once more, but you must wait for a new code to be generated. 
...
After typing ssh command, enter the 6-digit code verification code generated by the Google Authenticator app when prompted for the Verification Code. __Note that when you type the code or the password, nothing will be shown on the screen.__ If system accepts the code it will prompt you for password. If you made a mistake when typing 6-digit code and are prompted for Verification Code once again, wait for the new code to be generated.

_When Changing your Password (Mandatory Prompt when First Connecting): 
